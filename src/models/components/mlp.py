...
class MyData(Data):
    def __cat_dim__(self, key, value, *args, **kwargs):
        if key in ['modulewise_true_edges','layerwise_true_edges','signal_true_edges']:
            print(key,value)
...
def load_dataset(input_subdir="",
    ...
    **kwargs):
    if input_subdir is not None:
        ...
        loaded_events = [
            Data.from_dict(
                torch.load(event, map_location=torch.device("cpu")).__dict__)
...
def load_dataset(input_subdir="",
    ...
    **kwargs):
    if input_subdir is not None:
        ...
        loaded_events = []
        for event in all_events[:num_events]:
            val = Data.from_dict(torch.load(event, map_location=torch.device("cpu")).__dict__)
            val['modulewise_true_edges'] = val['modulewise_true_edges'].T
            val['layerwise_true_edges'] = val['layerwise_true_edges'].T
            val['signal_true_edges'] = val['signal_true_edges'].T
            loaded_events.append(val)
        """
        loaded_events = [
            MyData.from_dict(
                torch.load(event, map_location=torch.device("cpu")).__dict__)
...
def evaluate_set_metrics(edge_cut, model, trainer, dm):
    model.hparams.edge_cut = edge_cut
    test_results = trainer.predict(ckpt_path='best',datamodel=dm)
